...
const stickerListData: sticker[] = [
    {
        type: 'HAPPY',
        label: '행복해',
        desc: '힘겨운 시기에도 행복을 찾는 대견한 나! 미래의 나에게도 행복 에너지를 전달해볼까?',
    },
    {
        type: 'EXPECT',
        label: '기대돼',
        desc: '미래에 기대되는 일이 있구나! 두근거리는 마음을 담아 보내볼까?',
    },
    {
        type: 'SHY',
        label: '쑥쓰//',
        desc: '오늘 혹시 핑크빛..? 오늘의 핑크빛 마음을 미래의 나도 느끼면 좋겠어!',
    },
    {
        type: 'LOVE',
        label: '사랑해',
        desc: '세상에서 날 가장 사랑하는 나! 미래에 어려운 일이 생겨도 날 계속 사랑해주면 좋겠어!',
    },
    {
        type: 'UNHAPPY',
        label: '속상',
        desc: '누가 널 속상하게 했어! 누구야 데리고와! 지금은 속상하지만 나중에는 희망차다는 메시지, 나중에 보면 별거 아닐거야',
    },
    {
        type: 'SAD',
        label: '슬퍼...',
        desc: '혹시 울진 않았지? 오늘 슬픈 일을 훌훌 털어버리고 내일은 웃자!',
    },
    {
        type: 'SHOCK',
        label: '쇼킹해!',
    ...
    return (
        <StickerListWrapper>
            {stickerListData.map((sticker) => (
                <StickerItemWrapper key={sticker.type}>
                    <StickerWrapper>{sticker.type}</StickerWrapper>
                    <SpeechBubble>{sticker.label}</SpeechBubble>
                </StickerItemWrapper>
            ))}
        </StickerListWrapper>
    )
